...
import {
  FiStar, FiTrash2, FiCheck, FiX, FiArrowLeft,
  FiClock, FiEye, FiEyeOff, FiFilter, FiMessageSquare,
} from "react-icons/fi";
...
const CourseList = ({ onSelect }) => {
  ...
  useEffect(() => {
    api.get("/admin/student-reviews/courses")
  ...
  if (!courses.length) return (
    <div className="text-center py-16 text-gray-500">
      ...
      <p>এখনো কোনো course এ student review আসেনি।</p>
    </div>
  );
        ...
        <button key={c._id} onClick={() => onSelect(c)}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-4 mt-1.5 text-xs">
              <span className="text-gray-500">{c.counts.total} মোট</span>
              <span className="text-yellow-400">{c.counts.pending} pending</span>
              <span className="text-green-400">{c.counts.active} active</span>
            </div>
          </div>
          {c.counts.pending > 0 && (
            <span className="shrink-0 bg-yellow-500 text-gray-950 text-xs font-bold px-2.5 py-1 rounded-full">
              {c.counts.pending} নতুন
            </span>
          )}
        </button>
      ))}
...
// ── Review list for a specific course ────────────────────────────────────────
const CourseReviewList = ({ course, onBack }) => {
  const [data,    setData]    = useState({ course: null, reviews: [] });
  const [loading, setLoading] = useState(true);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get(`/admin/student-reviews/course/${course._id}${q}`);
      setData(r.data);
    } catch { toast.error("Load failed"); }
    finally { setLoading(false); }
  }, [course._id, filter]);
  ...
  const setStatus = async (review, status) => {
    ...
    try {
      await api.put(`/admin/student-reviews/${review._id}`, { status });
      toast.success(status === "active" ? "✅ Active করা হয়েছে" : status === "rejected" ? "❌ Reject করা হয়েছে" : "Updated");
      load();
  ...
  const deleteReview = async (review) => {
    ...
    setWorking(review._id);
    try {
      await api.delete(`/admin/student-reviews/${review._id}`);
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {r.status !== "active" && (
                    <button disabled={working === r._id} onClick={() => setStatus(r, "active")}
                      title="Active করো — public page এ দেখাবে"
                  ...
                  {r.status === "active" && (
                    <button disabled={working === r._id} onClick={() => setStatus(r, "pending")}
                      title="Inactive করো — public page থেকে সরাবে"
...
// ── Main page ─────────────────────────────────────────────────────────────────
const AdminStudentCourseReviews = () => {
  const [selectedCourse, setSelectedCourse] = useState(null);

  return (
    <DashboardLayout>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-white">Course Review</h2>
        <p className="text-gray-400 text-sm">
          Student-submitted course reviews — approve করলে public page এ দেখাবে।
        </p>
      </div>

      {selectedCourse ? (
        <CourseReviewList course={selectedCourse} onBack={() => setSelectedCourse(null)} />
      ) : (
...
export default AdminStudentCourseReviews;
